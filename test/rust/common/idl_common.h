...
public:
    ...
    int Ready(int argc, char** argv)
    {
        ...
        if (options.DoGenerateCode()) {
            ...
            if (options.GetTargetLanguage().Equals("rust")) {
                this->rustCodeGen_ = std::make_shared<RustCodeEmitter>(metadata.get());
                this->rustCodeGen_->SetDirectory(options.GetGenerationDirectory());
            }
        }
        return 0;
    }
    ...
    std::shared_ptr<RustCodeEmitter> rustCodeGen_ = nullptr;
};
