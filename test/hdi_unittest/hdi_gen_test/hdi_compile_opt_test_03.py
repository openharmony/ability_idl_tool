...
class hdi_compile_opt_test_01(Test):
    ...
    def set_idl_attr(self):
        ...
        command = f"{self._idl} --intf-type hdi {self._gen_langauge} -c -c "\
            f"--system {self._system_attr} -m {self._mode_attr} -c "\
            f"-r ohos.hdi:{self.get_working_dir()} -d {self.output_dir} -c "\
