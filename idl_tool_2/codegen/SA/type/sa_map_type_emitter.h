...
#ifndef OHOS_IDL_SA_MAPTYPE_EMITTER_H
#define OHOS_IDL_SA_MAPTYPE_EMITTER_H

#include "codegen/SA/sa_type_emitter.h"

namespace OHOS {
namespace Idl {
class SaMapTypeEmitter : public SaTypeEmitter {
public:
    ...
    std::string EmitRustType(bool pt = false, TypeMode mode = TypeMode::NO_MODE) const override;

    std::string EmitTsType(TypeMode mode = TypeMode::NO_MODE) const override;
    ...
    void EmitRustReadVar(const std::string &result, const std::string &name, StringBuilder &sb,
        const std::string &prefix) const override;

    void EmitRustWriteVar(const std::string &result, const std::string &name, StringBuilder &sb,
        const std::string &prefix) const override;
...
#endif // OHOS_IDL_SA_MAPTYPE_EMITTER_H