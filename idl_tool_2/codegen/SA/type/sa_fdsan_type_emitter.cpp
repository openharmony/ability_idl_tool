...
#include "sa_fdsan_type_emitter.h"

namespace OHOS {
namespace Idl {
TypeKind SaFdSanTypeEmitter::GetTypeKind()
{
    return TypeKind::TYPE_FILEDESCRIPTORSAN;
}

std::string SaFdSanTypeEmitter::EmitCppType(TypeMode mode) const
{
    switch (mode) {
...
void SaFdSanTypeEmitter::EmitCppWriteVar(const std::string &parcelName, const std::string &name, StringBuilder &sb,
    const std::string &prefix) const
{
    ...
    if (!isProxy) {
        ...
        sb.Append(prefix).Append(TAB).Append(TAB).AppendFormat("fdsan_close_with_tag(%s, %s);\n", name.c_str(),
            domainId_.c_str());
...
void SaFdSanTypeEmitter::EmitCppReadVar(const std::string &parcelName, const std::string &name, StringBuilder &sb,
